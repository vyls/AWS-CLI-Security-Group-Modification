#!/bin/bash
#input="/var/www/html/whitelist_sg.txt" #change the directory of the list of whitelist IP
input=$1
...
aws ec2 authorize-security-group-ingress --group-id $aws_security_group --protocol tcp --port $aws_whitelist_port --cidr $aws_whitelist_prefix
